...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
	...
	if(argmat->mat->rows!=self->mat->rows||argmat->mat->cols!=self->mat->cols){
		PyErr_SetString(PyExc_TypeError,"the dimension doesn't match!");
		return NULL;
	}
	Matrix61c* result = Matrix61c_new(&Matrix61cType,NULL,NULL);
	int get = allocate_matrix(&(result->mat),argmat->mat->rows,argmat->mat->cols);
	if(get<0){
		PyErr_SetString(PyExc_TypeError,"allocate fail");
		return NULL;
	}
	add_matrix(result->mat,self->mat,argmat->mat);
        result->shape = PyTuple_Pack(2,PyLong_FromLong(result->mat->rows),PyLong_FromLong(result->mat->cols));
...
static PyObject *Matrix61c_sub(Matrix61c* self, PyObject* args) {
	...
	Matrix61c* result = Matrix61c_new(&Matrix61cType,NULL,NULL);
	int res = allocate_matrix(&(result->mat),argmat->mat->rows,argmat->mat->cols);
	if(res<0){
		PyErr_SetString(PyExc_TypeError,"allocation fail");
		return NULL;
	}
	sub_matrix(result->mat,self->mat,argmat->mat);
	result->shape = PyTuple_Pack(2,PyLong_FromLong(result->mat->rows),PyLong_FromLong(result->mat->cols));
	return (PyObject *)result;
}
...
static PyObject *Matrix61c_multiply(Matrix61c* self, PyObject *args) {
	...
	Matrix61c* argmat = (Matrix61c*)args;
	if(argmat->mat->rows!=self->mat->cols){
		PyErr_SetString(PyExc_TypeError, "the dimensions should match!");
		return NULL;
	}
	Matrix61c* result = Matrix61c_new(&Matrix61cType,NULL,NULL);
	int res = allocate_matrix(&(result->mat),self->mat->rows,argmat->mat->cols);
	if(res<0){
		PyErr_SetString(PyExc_TypeError,"allocation fail");
		return NULL;
	}
	mul_matrix(result->mat,self->mat,argmat->mat);
	result->shape = PyTuple_Pack(2,PyLong_FromLong(result->mat->rows),PyLong_FromLong(result->mat->cols));
	return (PyObject *)result;
}
...
static PyObject *Matrix61c_neg(Matrix61c* self) {
    /* TODO: YOUR CODE HERE */
	Matrix61c* result = Matrix61c_new(&Matrix61cType,NULL,NULL);
	int res = allocate_matrix(&(result->mat),self->mat->rows,self->mat->cols);
	if(res<0){
		PyErr_SetString(PyExc_TypeError,"allocation fail");
		return NULL;
	}
...
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional) {
    /* TODO: YOUR CODE HERE */
	if(!PyLong_Check(pow)){
		PyErr_SetString(PyExc_TypeError, "pow is not valid");
		return NULL;
	}
	int power = PyLong_AsLong(pow);
	if(power<0){
		PyErr_SetString(PyExc_TypeError,"pow can't be negative");
		return NULL;
	}
	Matrix61c* result = Matrix61c_new(&Matrix61cType,NULL,NULL);
	int res = allocate_matrix(&(result->mat),self->mat->rows,self->mat->cols);
	...
	int indicator = pow_matrix(result->mat,self->mat,power);
	if(indicator!=0){
		PyErr_SetString(PyExc_TypeError,"pow dimension wrong");
		return NULL;
	}
	result->shape = PyTuple_Pack(2,PyLong_FromLong(result->mat->rows),PyLong_FromLong(result->mat->cols));
	return (PyObject *)result;
...
static PyObject *Matrix61c_set_value(Matrix61c *self, PyObject* args) {
	...
	int colsval;
	double valval;
	if(PyArg_UnpackTuple(args,"argument",3,3,&rowob,&colob,&valob)){
		if(!PyLong_Check(rowob)||!PyLong_Check(colob)||!PyFloat_Check(valob)){
			PyErr_SetString(PyExc_TypeError,"argument type mismatched");
			return NULL;
		}
		else if((PyLong_AsLong(rowob)<0)||(PyLong_AsLong(colob)<0)||(PyLong_AsLong(rowob)>=self->mat->rows)||(PyLong_AsLong(colob)>=self->mat->cols)){
			PyErr_SetString(PyExc_IndexError,"row of col can't be less than zero!");
			return NULL;
		}
		rowsval = PyLong_AsLong(rowob);
		colsval = PyLong_AsLong(colob);
		valval = PyFloat_AsDouble(valob);
		set(self->mat,rowsval,colsval,valval);
	}
	else{
		PyErr_SetString(PyExc_TypeError,"wrong number of argument");
		return NULL;
	}
	Py_RETURN_NONE;
}
...
static PyObject *Matrix61c_get_value(Matrix61c *self, PyObject* args) {
	...
	int colsval;
	double result;
	if(PyArg_UnpackTuple(args,"argument",2,2,&rowob,&colob)){
		if(!PyLong_Check(rowob)||!PyLong_Check(colob)){
			PyErr_SetString(PyExc_TypeError,"argument type mismatched");
			return NULL;
		}
		else if((PyLong_AsLong(rowob)<0)||(PyLong_AsLong(colob)<0)||(PyLong_AsLong(rowob)>=self->mat->rows)||(PyLong_AsLong(colob)>=self->mat->cols)){
			PyErr_SetString(PyExc_IndexError,"row or col can't be less than zero!");
			return NULL;
		}
		rowsval = PyLong_AsLong(rowob);
		colsval = PyLong_AsLong(colob);
		result = get(self->mat,rowsval,colsval);
	}
	else{
		PyErr_SetString(PyExc_TypeError,"wrong number of argument");
		return NULL;
...
static PyMethodDef Matrix61c_methods[] = {
    /* TODO: YOUR CODE HERE */
	{"set",(PyCFunction)Matrix61c_set_value,METH_VARARGS,"this func set the desired position in the mat"},
	{"get",(PyCFunction)Matrix61c_get_value,METH_VARARGS,"this func return the desired value in the mat"},
    {NULL, NULL, 0, NULL}
};
